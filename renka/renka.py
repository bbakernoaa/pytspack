...
class TsPack:
    ...
    def interpolate(self, x, y, tension=0.0):
        ...
        if ier.value != 0: raise ValueError(f"TSPACK Error: {ier.value}")
...
class SphericalMesh:
    def __init__(self, lats, lons):
        ...
        _lib.trans.argtypes = [c_int, c_double_p, c_double_p, c_double_p, c_double_p, c_double_p]
        ...
        _lib.trmesh.argtypes = [c_int, c_double_p, c_double_p, c_double_p,
                                c_int_p, c_int_p, c_int_p, POINTER(c_int),
                                c_int_p, c_int_p, c_double_p, POINTER(c_int)]

        ier = c_int()
        _lib.trmesh(self.n, self.x, self.y, self.z,
                    self.list, self.lptr, self.lend, byref(self.lnew),
                    np.zeros(self.n, dtype=np.int32), np.zeros(self.n, dtype=np.int32),
                    np.zeros(self.n, dtype=np.float64), byref(ier))

        if ier.value < 0: raise RuntimeError(f"TRMESH Error: {ier.value}")
        self._bind()
    ...
    def _bind(self):
        try:
            _lib.unif.argtypes = [c_int, c_int_p, c_int, c_double_p, c_double_p, c_double_p, c_double_p,
                                  c_int_p, c_int_p, c_int_p, c_int, c_double_p,
                                  c_int, c_int, c_int, c_double_p, c_double_p,
                                  c_int, c_double, c_double_p, POINTER(c_int)]
    ...
    def interpolate(self, values, grid_lats, grid_lons):
        ...
        _lib.unif(0, np.zeros(1, dtype=np.int32), self.n, self.x, self.y, self.z, vals, self.list, self.lptr, self.lend,
                  0, np.zeros(1, dtype=np.float64), ni, ni, nj, g_lat, g_lon,
                  0, 0.0, ff, byref(ier))
